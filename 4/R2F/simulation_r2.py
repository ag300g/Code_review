...
class DailyEvaluate:
    def __init__(self, sku_list, dc_list, period_length,
        ...
        self.allcost = 0
        self.ordering_cost1 = 0
        self.shortage_cost1 = 0
        self.holding_cost1 = 0
        self.extra_shipping_cost1 = 0
    ...
    def daily_update(self, inventory_decision, t):
        ...
        _shortage = np.floor(self.true_demand[t][1:] - _fdc_sales)
        a = np.zeros((5, 1000))
        for i in range(5):
            a[i, :] = self.sku_stockout_cost * self.abandon_rate[i]

        cost1 = _shortage * a
        cost_1 = cost1.sum()

        # print('fdc_shortage_cost_1',cost_1)
        ...
        if t < self.period_length - self.replenishment_leadtime:
            ...
            self.ordering_cost1 += np.count_nonzero(new_replenishment) * self.fixed_replenish_cost_per_order
            # update in-transit replenishment inventory
            self.intransit_replenishment[t + self.replenishment_leadtime - 1] = new_replenishment

        # print('ordering_cost',self.ordering_cost)

        self.holding_cost = (self.inv * self.sku_holding_cost).sum() + (
                    self.intransit_transshipment * self.sku_holding_cost).sum()
        _lost_sale = self.true_demand[t].sum(axis=0) - (
                    _fdc_sales.sum(axis=0) + _transship_sales.sum(axis=0) + _rdc_sales)
        self.holding_cost1 += (self.inv * self.sku_holding_cost).sum() + (
                    self.intransit_transshipment * self.sku_holding_cost).sum()
        _lost_sale = self.true_demand[t].sum(axis=0) - (
                    _fdc_sales.sum(axis=0) + _transship_sales.sum(axis=0) + _rdc_sales)
        self.shortage_cost = (_lost_sale * self.sku_stockout_cost).sum()
        self.shortage_cost1 += (_lost_sale * self.sku_stockout_cost).sum()
        # cost_2=self.shortage_cost-cost_1
        # print('rdc.shortage_cost',cost_2)

        # print('shortage_cost',self.shortage_cost)

        self.extra_shipping_cost = self.extra_shipping_cost_per_unit * np.maximum(_rdc_sales - self.true_demand[t][0],
                                                                                  np.zeros(len(self.sku_list)).astype(
                                                                                      int)).sum()

        self.extra_shipping_cost1 += self.extra_shipping_cost_per_unit * np.maximum(_rdc_sales - self.true_demand[t][0],
                                                                                    np.zeros(len(self.sku_list)).astype(
                                                                                        int)).sum()
        # print('extra_shipping_cost',self.extra_shipping_cost)

        # myfile = open('dicision.txt','a')
        # print(t,file = myfile)
        print('shortage_cost', self.shortage_cost)  # ,file = myfile)
        print('holding_cost', self.holding_cost)  # ,file = myfile)

        # myfile.close()
        self.allcost += self.holding_cost + self.extra_shipping_cost + self.shortage_cost + self.ordering_cost
...
if __name__ == '__main__':
    ...
    # datelist=pd.date_range(date(2018,1,25), periods=31)

    '''MOCK DATA. RANDOMLY GENERATED'''

    true_demand = pd.read_csv("y_real2_cv.csv").set_index(['dc_id', 'item_sku_id'])
    true_demand = true_demand.values
    true_demand = true_demand.T.reshape(61, 6, 1000)
    # true_demand = np.random.randint(1000, size = (period_length, len(dc_list), len(sku_list)))
    ...
    score = instance.extra_shipping_cost1 + instance.shortage_cost1 + instance.ordering_cost1 + instance.holding_cost1
    myfile = open('dicision.txt', 'a')
    print('shortage', instance.shortage_cost1, file=myfile)
    print('holding', instance.holding_cost1, file=myfile)
    print('score ', score, file=myfile)
    myfile.close()
    print('shortage', instance.shortage_cost1)
    print('holding', instance.holding_cost1)
