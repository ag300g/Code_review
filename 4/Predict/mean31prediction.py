...
def prepare_prom(df, promo_df, t2017, is_train=True, name_prefix=None):
    ...
    if is_train:
        y = df[
            pd.date_range(t2017, periods=31)
        ].values
        y = np.mean(y, axis=1)
        return X, y
...
df_preds.reset_index().to_csv('meantwo_val-cv29.csv', index=False)
...
df_preds.index.set_names(["dc_id", "item_sku_id", "date"], inplace=True)
# df_preds["quantity"] = np.clip(np.expm1(df_preds["quantity"]), 0, 1000)
df_preds.reset_index().to_csv('meantwo_test_cv29.csv', index=False)
